 ...
 * - NbtAccounter(size) constructor instead of NbtAccounter.create(size)
 * - template.load(nbt) instead of template.load(holderLookup, nbt)
 * - net.minecraftforge.client.model.data.ModelData instead of NeoForge's
 * - net.minecraftforge.api.distmarker instead of net.neoforged.api.distmarker
 */
@OnlyIn(Dist.CLIENT)
object SchematicPreviewRenderer {
     ...
     * Uses fixed isometric angles (30 deg X, -45 deg Y) and auto-fit zoom.
     */
    ...
    fun renderPreview(
        ...
        // Rotation (isometric default: 30 deg X, -45 deg Y)
    ...
    fun loadFromNbtBytes(key: String, nbtBytes: ByteArray): Boolean {
        ...
        try {
            ...
            // Forge 1.20.1: NbtAccounter constructor, not NbtAccounter.create()
            val nbt = NbtIo.read(stream, NbtAccounter(0x20000000L))
            val template = StructureTemplate()
            // Forge 1.20.1: template.load(HolderGetter<Block>, CompoundTag)
