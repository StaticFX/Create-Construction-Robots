 ...
 *
 * Forge 1.20.1 override: Uses IItemHandler (Forge) instead of NeoForge's IItemHandler,
 * and isSameItemSameTags instead of isSameItemSameComponents.
 */
class PortableBeeHive(val player: Player) : BeeHive, LogisticsPort {
    ...
    override fun hasItemStack(stack: ItemStack): Boolean {
        ...
        for (i in 0 until player.inventory.containerSize) {
            ...
            if (!slot.isEmpty && ItemStack.isSameItemSameTags(slot, stack) && slot.count >= stack.count) {
                return true
            }
        }
        return false
