 ...
 *
 * Forge 1.20.1 override: Uses FriendlyByteBuf instead of RegistryFriendlyByteBuf,
 * and NBT-based storage instead of DataComponents/ItemContainerContents.
 */
class BeehiveContainer : AbstractContainerMenu {
    ...
    constructor(type: MenuType<*>, containerId: Int, playerInventory: Inventory, extraData: FriendlyByteBuf)
    ...
    constructor(type: MenuType<*>, containerId: Int, playerInventory: Inventory, backpackStack: ItemStack)
        ...
        // Load contents from the backpack item via NBT
        val items = PortableBeehiveItem.loadItems(backpackStack)
    ...
    private fun saveBackpackContents() {
        // Save the container contents back to the backpack item via NBT
        val items = mutableListOf<ItemStack>()
        for (i in 0 until backpackInventory.containerSize) {
            items.add(backpackInventory.getItem(i))
        }
        PortableBeehiveItem.saveItems(backpackStack, items)
    }
