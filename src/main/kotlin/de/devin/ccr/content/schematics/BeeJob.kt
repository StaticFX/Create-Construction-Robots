...
data class BeeJob(
    ...
    fun claimNextTask(robotId: Int): BeeTask? {
        val task = tasks.firstOrNull { it.status == BeeTask.TaskStatus.PENDING }
    ...
    fun getNextTask(): BeeTask? {
        return tasks.firstOrNull { it.status == BeeTask.TaskStatus.PENDING }
    }
    ...
    fun isComplete(): Boolean {
        return tasks.all { it.status == BeeTask.TaskStatus.COMPLETED || it.status == BeeTask.TaskStatus.CANCELLED }
    }
    ...
    fun cancel() {
        ...
        tasks.forEach { 
            if (it.status == BeeTask.TaskStatus.PENDING || it.status == BeeTask.TaskStatus.IN_PROGRESS) {
                it.cancel()
            }
        }
    }
    ...
    fun getProgress(): Float {
        ...
        val completed = tasks.count { it.status == BeeTask.TaskStatus.COMPLETED }
    ...
    fun getRemainingTaskCount(): Int {
        return tasks.count { it.status == BeeTask.TaskStatus.PENDING || it.status == BeeTask.TaskStatus.IN_PROGRESS }
    }
}
