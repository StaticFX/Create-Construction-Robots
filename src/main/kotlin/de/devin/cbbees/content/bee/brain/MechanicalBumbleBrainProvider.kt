    ...
    fun makeBrain(brain: Brain<MechanicalBumbleBeeEntity>): Brain<MechanicalBumbleBeeEntity> {
        brain.addActivity(
            Activity.CORE, ImmutableList.of(
                ...
                Pair.of(2, UpdateBumbleStatusBehavior()),
                Pair.of(3, BumbleStuckSafetyBehavior())
            )
        )
        ...
        brain.addActivityWithConditions(
            Activity.WORK,
            ImmutableList.of(
                Pair.of(0, BumbleRechargeSpringBehavior()),
                Pair.of(1, PickUpFromSourceBehavior()),
                Pair.of(2, FlyToTargetPortBehavior()),
                Pair.of(3, DepositAtTargetBehavior())
            ),
            setOf(Pair.of(BeeMemoryModules.TRANSPORT_TASK.get(), MemoryStatus.VALUE_PRESENT))
        )

        brain.addActivityWithConditions(
            Activity.REST,
            ImmutableList.of(
                Pair.of(1, BumbleEnterHiveBehavior()),
                Pair.of(2, BumbleSetHiveTargetBehavior())
            ),
            setOf(Pair.of(BeeMemoryModules.TRANSPORT_TASK.get(), MemoryStatus.VALUE_ABSENT))
        )
